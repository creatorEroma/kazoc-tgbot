...
async def render_deal(o, u):
    ...
    elif o["status"] in (ST_WORK, ST_REOPEN): st_rows = [[btn("✅ Завершить", f"deal:st:{o['id']}:win")], [btn("❌ Неудачная", f"deal:st:{o['id']}:loss")]]
...
async def api_update_company(id: int, payload: CompanyPayload, u = Depends(get_current_user)):
    ...
    asyncio.create_task(sync_company_to_bitrix(id))
...
async def api_update_deal(id: int, payload: DealPayload, u = Depends(get_current_user)):
    ...
    asyncio.create_task(sync_deal_to_bitrix(id))
...
async def api_update_deal_status(id: int, action: str, u = Depends(get_current_user)):
    ...
    if not d or not can_access_deal(u, d):
        raise HTTPException(status_code=403, detail="Нет доступа")
    new_status = {"work": ST_WORK, "loss": ST_LOSS, "reopen": ST_REOPEN}.get(action)
    if not new_status:
...
async def api_delete_deal(id: int, u = Depends(get_current_user)):
    ...
    if d.get("bitrix_id"):
        try:
            async with httpx.AsyncClient() as client:
                await client.post(f"{BITRIX_URL}crm.deal.delete.json", json={"id": d["bitrix_id"]})
        except Exception:
            pass
    async with pool.acquire() as conn:
